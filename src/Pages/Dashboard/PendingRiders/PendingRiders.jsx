import { useState } from "react";
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import useAxios from "../../../Hooks/useAxios";

const PendingRider = () => {
  const [selectedRider, setSelectedRider] = useState(null);
  const queryClient = useQueryClient();
  const axiosSecure = useAxios()
  // 🔹 Fetch Pending Riders
  const { data: riders = [], isLoading } = useQuery({
    queryKey: ["riders-pending"],
    queryFn: async () => {
      const res = await axiosSecure.get('/riders/pending?status=pending')
      return res.data
    }, 
  });
//   console.log(riders)

  // 🔹 Approve Mutation
  const approveMutation = useMutation({
    mutationFn: async (id) => {
      return fetch(`http://localhost:5000/riders/approve/${id}`, {
        method: "PATCH",
      });
    },
    onSuccess: () => {
      queryClient.invalidateQueries(["pendingRiders"]);
      setSelectedRider(null);
    },
  });

  // 🔹 Reject Mutation
  const rejectMutation = useMutation({
    mutationFn: async (id) => {
      return fetch(`http://localhost:5000/riders/reject/${id}`, {
        method: "PATCH",
      });
    },
    onSuccess: () => {
      queryClient.invalidateQueries(["pendingRiders"]);
      setSelectedRider(null);
    },
  });

  if (isLoading) {
    return <div className="text-center mt-10">Loading...</div>;
  }

  return (
    <div className="p-6">
      <h2 className="text-2xl font-bold mb-4">Pending Riders</h2>

      {/* 🔹 Table */}
      <div className="overflow-x-auto bg-white shadow rounded-lg">
        <table className="table w-full">
          <thead className="bg-gray-100">
            <tr>
              <th>#</th>
              <th>Name</th>
              <th>Email</th>
              <th>Region</th>
              <th>District</th>
              <th>Action</th>
            </tr>
          </thead>

          <tbody>
            {riders.map((rider, index) => (
              <tr key={rider._id}>
                <td>{index + 1}</td>
                <td>{rider.name}</td>
                <td>{rider.email}</td>
                <td>{rider.region}</td>
                <td>{rider.district}</td>
                <td>
                  <button
                    className="btn btn-sm text-black btn-primary"
                    onClick={() => setSelectedRider(rider)}
                  >
                    View
                  </button>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>

      {/* 🔹 Modal */}
      {selectedRider && (
        <div className="fixed inset-0 bg-[#ffffffbe] flex items-center justify-center z-50">
          <div className="bg-white rounded-lg w-[420px] p-6 relative shadow-lg">
            <h3 className="text-xl font-bold mb-4">Rider Details</h3>

            <div className="space-y-2 text-sm">
              <p><b>Name:</b> {selectedRider.name}</p>
              <p><b>Email:</b> {selectedRider.email}</p>
              <p><b>Phone:</b> {selectedRider.phone}</p>
              <p><b>Region:</b> {selectedRider.region}</p>
              <p><b>District:</b> {selectedRider.district}</p>
              <p><b>Address:</b> {selectedRider.address}</p>
              <p><b>Status:</b> {selectedRider.status}</p>
            </div>

            {/* 🔹 Buttons */}
            <div className="flex justify-end gap-3 mt-6">
              <button
                className="btn btn-sm"
                onClick={() => setSelectedRider(null)}
              >
                Close
              </button>

              <button
                className="btn btn-sm btn-error"
                onClick={() => rejectMutation.mutate(selectedRider._id)}
              >
                Cancel
              </button>

              <button
                className="btn btn-sm btn-success"
                onClick={() => approveMutation.mutate(selectedRider._id)}
              >
                Accept
              </button>
            </div>
          </div>
        </div>
      )}
    </div>
  );
};

export default PendingRider;
